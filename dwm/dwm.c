...
{
	/* focus-follows-mouse disabled: focus only changes on click (see buttonpress).
	 * Kept as a stub because enternotify is still wired into the event handler table. */
	return;
}
...
shiftview(const Arg *arg)
{
	...
	if (arg->i > 0) /* view the tag(s) to the right */
		a.ui = ((a.ui << arg->i) | (a.ui >> (LENGTH(tags) - arg->i))) & TAGMASK;
	else            /* view the tag(s) to the left */
		a.ui = ((a.ui >> -arg->i) | (a.ui << (LENGTH(tags) + arg->i))) & TAGMASK;
	view(&a);
}
